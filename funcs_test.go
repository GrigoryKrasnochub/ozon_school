...
const (
	...
	test4BeforeCut = "  On  my        home world   "
	test4AfterCut  = "On my home world "
	test5BeforeCut = "**On**my********home*world***"
	test5AfterCut  = "On*my*home*world*"
)
...
func testCutSymbolDuplicationExtended(beforeCut, afterCut []rune, symToCut rune, t *testing.T) {
	sourceStringCopy := string(beforeCut)
	result := cutSymbolDuplication(symToCut, beforeCut)
	if string(result) != string(afterCut) {
		t.Fatalf("expected: %s; fact: %s", string(afterCut), string(result))
	}
	t.Logf("success! source string: \"%s\"; expected result: \"%s\"; result: \"%s\"; cut symbol: \"%s\";", sourceStringCopy, string(afterCut), string(result), string(symToCut))
}
...
func TestCutSymbolDuplication4(t *testing.T) {
	testCutSymbolDuplication([]rune(test4BeforeCut), []rune(test4AfterCut), t)
}

func TestCutSymbolDuplication5(t *testing.T) {
	testCutSymbolDuplicationExtended([]rune(test5BeforeCut), []rune(test5AfterCut), '*', t)
}
